...
interface SupplierErrorScreenProps {
    onRetry?: () => void;
    error?: string;
}

export default function SupplierErrorScreen({ onRetry, error }: SupplierErrorScreenProps) {
    return (
        <Main>
                ...
                <Column mh={26} gv={20} pv={20} ph={20} style={{ backgroundColor: '#fff', borderRadius: 12 }}>
                    <Column align="center" gv={12}>
                        ...
                        <Label align='center' color={colors().color.muted}>
                            {error || 'Não foi possível carregar os fornecedores. Verifique sua conexão e tente novamente.'}
                        </Label>
                    </Column>
