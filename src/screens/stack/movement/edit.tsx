...
import { Main, Button, Column, Input, ScrollVertical, Tabs, Tipo, Title, Row, colors, useMutation, useToast, useFetch, Loader, Icon, Label } from "@/ui";
import { Pressable, KeyboardAvoidingView, FlatList, Text } from "react-native";
import { Check } from 'lucide-react-native';
import { MovementService, UpdateMovementRequest } from "src/services/movement";
...
export default function MoveEditScreen({ navigation, route }) {
    ...
    const { data: movement, isLoading: loadingMovement } = useFetch({
        key: `movement-${id}`,
        fetcher: async () => {
    ...
    useEffect(() => {
        if (movement) {
            ...
            reset({
                type: movement.type.toLowerCase() as 'entrada' | 'saida' | 'perda',
                quantity: movement.quantity.toString(),
                price: movement.price?.toString() || "",
                productId: movement.productId,
                supplierId: movement.supplierId || "",
                batch: movement.batch || "",
                expiration: formatDateToDDMMYYYY(movement.expiration || ""),
                note: movement.note || "",
            });
        }
    }, [movement, reset]);
    ...
    const updateMovementMutation = useMutation({
        mutationFn: async (params: MovementFormData) => {
            ...
            const movementParams: UpdateMovementRequest = {
            ...
            return await MovementService.update(id, movementParams);
        },
        onSuccess: () => {
            toast.showSuccess('Movimentação atualizada com sucesso!');
        ...
        onError: (error) => {
            console.log(error);
            toast.showError(error.message || 'Erro ao atualizar movimentação');
        }
    });
            ...
            <KeyboardAvoidingView behavior="padding">
                ...
                {loadingMovement ? (
                    ...
                    <ScrollVertical>
                        ...
                        {tab === "Observação" && <Observation control={control} errors={errors} isLoading={updateMovementMutation.isLoading} onSubmit={handleSubmit(onSubmit)} />}
                    </ScrollVertical>
                )}
