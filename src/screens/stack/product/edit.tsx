...
import { Main, Button, Column, Input, ScrollVertical, Tabs, Medida, Status, Title, Row, colors, Loader, useFetch, useMutation, useToast, Icon, Label } from "@/ui";
...
export default function ProductEditScreen({ route, navigation }) {
    ...
    const values = ['UNIDADE', 'KG', 'L', 'ML', 'M', 'CM', 'MM', 'UN', 'DZ', 'CX', 'PCT', 'KIT', 'PAR', 'H', 'D'];
    ...
    useEffect(() => {
        if (product) {
            ...
            setValue("referencePrice", product.referencePrice?.toString() || "");
            setValue("alertPercentage", product.alertPercentage?.toString() || "10");
            setValue("stockMin", product.stockMin?.toString() || "");
            setValue("stockMax", product.stockMax?.toString() || "");
            setValue("status", product.status);
            setValue("unitOfMeasure", product.unitOfMeasure || "UNIDADE");
            setValue("categoryIds", product.categoryId ? [product.categoryId] : []);
        }
    }, [product, setValue]);
    ...
    const updateProductMutation = useMutation({
        mutationFn: async (params: ProductFormData) => {
            const updateParams = {
                ...
                categoryId: params.categoryIds[0], // Para edição, mantemos apenas uma categoria
                stockMin: parseInt(params.stockMin),
                stockMax: parseInt(params.stockMax),
                alertPercentage: parseInt(params.alertPercentage),
                status: params.status,
            };
            return await ProductService.update(id, updateParams);
        },
        onSuccess: () => {
    ...
    return (
        ...
        <KeyboardAvoidingView behavior="padding">
            ...
            {loadingProduct ?
                ...
                <ScrollVertical>
                        {tab === "Sobre" && <About control={control} errors={errors} values={values} settab={setTab} />}
                        {tab === "Categorias" && <Categories errors={errors} settab={setTab} setValue={setValue} watch={watch} />}
                        {tab === "Estoque" && <Stock control={control} errors={errors} isLoading={updateProductMutation.isLoading} onSubmit={handleSubmit(onSubmit)} handleDelete={handleDelete} />}
                </ScrollVertical>}
...
const About = ({ control, errors, settab, values }: AboutProps) => {
...
const Categories = ({ errors, settab, setValue, watch }: CategoriesProps) => {
...
const Stock = ({ control, errors, isLoading, onSubmit, handleDelete }: StockProps) => {
